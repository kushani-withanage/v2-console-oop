...
class StudentManagementSystem {
    ...
    public static Batch[] batchArray = new Batch[0];

    // Initializes and creates Student objects from hardcoded data arrays.
    ...
    public static void initializeStudents() {
        ...
        String[] nameArray = {
                ...
                "Bandara Karunaratne", "Fernando Perera", "De Silva Silva", "Rajapaksha Gunawardena",
                "Herath Weerasinghe",
                "Karunaratne Dias", "Jayasinghe Silva", "Senanayake Abeysekera", "Silva Jayasinghe",
                "Rathnayake Kumara",
                "Gunawardena Kumara", "Rajapaksha Silva", "Perera Jayasinghe", "Silva Ekanayake", "Dias Senanayake",
                "Herath Abeysekera", "Rathnayake Fernando", "Kumara Herath", "Weerasinghe Silva",
                "Senanayake Karunaratne",
                "Abeysekera Silva", "Bandara Gunawardena", "Karunaratne Weerasinghe", "Perera Herath", "Fernando Dias",
                "Weerasinghe Gunawardena", "Rathnayake Kumara", "Senanayake Fernando", "Silva Bandara",
                "Herath Rajapaksha",
                ...
                "Kumara Karunaratne",
                "Dias Rajapaksha", "Herath Perera", "Rathnayake Gunawardena", "Ekanayake Jayasinghe",
                "Gunawardena Silva",
                ...
                "Abeysekera Silva",
                "Ekanayake Bandara", "Rajapaksha Fernando", "Gunawardena Weerasinghe", "Kumara Karunaratne",
                "Silva Dias",
                "Perera Weerasinghe", "Karunaratne Rajapaksha", "Jayasinghe Silva", "Rathnayake Perera",
                "Silva Ekanayake",
                "Silva Karunaratne", "Herath Fernando", "Kumara Jayasinghe", "Weerasinghe Perera",
                "Abeysekera Rajapaksha",
                "Rathnayake Karunaratne", "Ekanayake Bandara", "Gunawardena Perera", "Silva Wijesinghe",
                "Rajapaksha Jayasinghe",
                "Rathnayake Fernando", "Karunaratne Kumara", "Perera Silva", "Gunawardena Ekanayake",
                "Bandara Rajapaksha",
                "Silva Herath", "Rathnayake Weerasinghe", "Perera Gunawardena", "Herath Karunaratne",
                "Silva Rajapaksha",
        ...
        // Create Student array with combined data
        studentsArray = new Student[regNoArray.length];
    ...
    // Initializes and creates Batch objects from hardcoded data arrays.
    public static void initializeBatches() {
        // Batch data
        String[] batchNameArray = { "105", "106", "107", "108", "109", "110" };
        int[] batchStatusArray = { 0, 0, 0, 0, 1, 1 };
        int[] batchStudentCountArray = { 25, 25, 25, 25, 25, 25 };

        for (int i = 0; i < batchStudentCountArray.length; i++) {
            batchArray[i] = new Batch(
                    batchNameArray[i], batchStatusArray[i], batchStudentCountArray[i]);
        }
    }
    ...
    public static void addBatch() {
        ...
        tempBatchArray[tempBatchArray.length - 1] = new Batch(batch, 1, 0);
        tempBatchArray = batchArray;
    ...
    // find Batch
    public static int findBatch(String batchId) {
        for (int i = 0; i < batchArray.length; i++) {
            if (batchId.equals(batchArray[i].getBatchId())) {
                return i;
            }
        }
        return -1;
    ...
    public static void updateBatch() {
        ...
        System.out.print("Enter Batch Number : ");
        String batch = input.nextLine();
        int index = findBatch(batch);
        if (index != -1) {
            System.out.println("Current Status : "
                    + (batchArray[index].getStatus() == 1 ? "ENROLLMENT OPEN" : "ENROLLMENT CLOSED"));
            if (batchArray[index].getStatus() == 0) {
                System.out.println("\nThis batch is already CLOSED and cannot be updated.");
                L: do {
    ...
    public static void viewBatch() {
        ...
        for (int i = 0; i < batchArray.length; i++) {
            String line = String.format(
                    "%-5d%-12s%-15d%-20s",
                    (i + 1),
                    batchArray[i].getBatchId(),
                    batchArray[i].getStudentCount(),
                    (batchArray[i].getStatus() == 1 ? "ENROLLMENT OPEN" : "ENROLLMENT CLOSED"));
    ...
    public static void studentRegistrationReport() {
        ...
        for (int i = 0; i < tempstuArray.length - 1; i++) {
            for (int j = 0; j < tempstuArray.length - 1 - i; j++) {
                if (tempstuArray[j].getName().compareToIgnoreCase(tempstuArray[j + 1].getName()) > 0) {
                    // Swap students reference
                    Student stu = tempstuArray[j];
                    tempstuArray[j] = tempstuArray[j + 1];
                    tempstuArray[j + 1] = tempstuArray[j];
                }
            }
        }
        for (int i = 0; i < tempstuArray.length; i++) {
            String line = String.format(
                    "%-5d%-18s%-40s%-14s%15d%15d%15.2f",
                    (i + 1),
                    tempstuArray[i].getRegNo(),
                    tempstuArray[i].getName(),
                    tempstuArray[i].getNic(),
                    tempstuArray[i].getPrf(),
                    tempstuArray[i].getDbms(),
                    tempstuArray[i].calculateGPA()
            );
            System.out.println(line);
        }
        L2: do {
    ...
    public static void batchwiseStudentReport() {
        ...
        for (int i = 0; i < batchArray.length; i++) {
            System.out.println("[" + (i + 1) + "] " + batchArray[i].getBatchId() + " Batch");
        }
        System.out.println("\n[" + (i + 1) + "] " + "Exit");
        System.out.print("\n\nEnter an option to continue > ");
        int option = input.nextInt();
        if (option < batchArray.length) {
            batchReportPrint(batchArray[option - 1]);
        } else {
            exit();
        }
    }
        ...
        for (int i = 0; i < studentsArray.length; i++) {
            if (batch.getBatchId().equals(studentsArray[i].getRegNo().substring(4, 7))) {
                String line = String.format(
                        "%-5d%-18s%-40s%-14s%15d%15d%15.2f",
                        no,
                        studentsArray[i].getRegNo(),
                        studentsArray[i].getName(),
                        studentsArray[i].getNic(),
                        studentsArray[i].getPrf(),
                        studentsArray[i].getDbms(),
                        studentsArray[i].calculateGPA());
                System.out.println(line);
                no++;
            }
        }
    ...
    public static void industryTrainingEligibility() {
        ...
        for (int i = 0; i < studentsArray.length; i++) {
            if (studentsArray[i].getPrf() == -1 || studentsArray[i].getPrf() == -2 || studentsArray[i].getDbms() == -1
                    || studentsArray[i].getDbms() == -2 || studentsArray[i].getPrf() < 50
                    || studentsArray[i].getDbms() < 50 || studentsArray[i].calculateGPA() < 3.25) {
                continue;
            } else {
                String line = String.format(
                        "%-5d%-18s%-40s%-14s%15d%15d%15.2f",
    ...
    // Main methos for manage functionality
